...
gcloud compute instances create lamp-1-vm \
    ...
    --tags=http-server \
    --create-disk=auto-delete=yes,boot=yes,device-name=lamp-1-vm,image=projects/debian-cloud/global/images/debian-12-bookworm-v20250311,mode=rw,size=10,type=pd-balanced \
...
gcloud compute scp prepare_disk.sh lamp-1-vm:/tmp \
    --project=$DEVSHELL_PROJECT_ID \
    --zone=$ZONE \
    --quiet
...
# Instance ID
export INSTANCE_ID=$(gcloud compute instances list \
    --filter=lamp-1-vm \
    --zones $ZONE \
    --format="value(id)")
